...
export function DeploysPage() {
  ...
  const loadDeploys = async () => {
    try {
      setLoading(true);
      const data = await apiClient.getDeploys();
  ...
  const handleDelete = async (deployId: string) => {
    ...
    try {
      await apiClient.deleteDeploy(deployId);
  ...
  const columnHelper = createColumnHelper<Deploy>();
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("type", {
        ...
        size: 140,
        cell: (info) => (
          <Badge type={getTypeBadgeColor(info.getValue())}>
            {info.getValue()}
          </Badge>
        ),
      }),
      columnHelper.accessor("environment", {
        ...
        size: 140,
        cell: (info) => (
          <Badge environment={getEnvironmentBadgeColor(info.getValue())}>
            {info.getValue()}
          </Badge>
        ),
      }),
      columnHelper.accessor("region", {
        ...
        size: 140,
        cell: (info) => (
          <span style={{ color: "$textMuted" }}>{info.getValue() || "-"}</span>
        ),
      }),
      ...
      columnHelper.display({
        ...
        size: 220,
        cell: (info) => {
          const deploy = info.row.original;
          return (
            <ActionCell>
              <Button
                variant="secondary"
                onClick={() => navigate(`/deploys/${deploy.id}`)}
                title="Visualizar"
              >
                <FiEye size={16} />
                Ver
      ...
      <PageContainer>
        <HeaderSection>
          ...
          <ConditionalRender requiredLevel={30}>
            <Button variant="primary" onClick={() => navigate("/deploys/new")}>
              + Novo Deploy
            </Button>
          </ConditionalRender>
        </HeaderSection>
        ...
        <TableContainer>
          {loading ? (
            ...
            <EmptyState>Nenhum deploy encontrado</EmptyState>
          ) : (
            <>
              <StyledTable>
                <thead>
