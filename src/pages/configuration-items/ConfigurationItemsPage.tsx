...
export function ConfigurationItemsPage() {
  ...
  const loadConfigurationItems = async () => {
    try {
      setLoading(true);
      const data = await apiClient.getConfigurationItems();
  ...
  const handleDelete = async (configItemId: string) => {
    if (!window.confirm("Tem certeza que deseja deletar este recurso?")) {
      return;
    }

    try {
      await apiClient.deleteConfigurationItem(configItemId);
  ...
  const columnHelper = createColumnHelper<ConfigurationItem>();
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.display({
        ...
        size: 220,
        cell: (info) => {
          const configItem = info.row.original;
          return (
            <ActionCell>
              <Button
                variant="secondary"
                onClick={() =>
                  navigate(`/configuration-items/${configItem.id}`)
                }
                title="Visualizar"
              >
                <FiEye size={16} />
      ...
      <PageContainer>
        <HeaderSection>
          ...
          <ConditionalRender requiredLevel={30}>
            <Button
              variant="primary"
              onClick={() => navigate("/configuration-items/new")}
            >
              + Novo Recurso
            </Button>
          </ConditionalRender>
        ...
        <TableContainer>
          {loading ? (
            ...
            <EmptyState>Nenhum recurso encontrado</EmptyState>
          ) : (
            <>
              <StyledTable>
                <thead>
